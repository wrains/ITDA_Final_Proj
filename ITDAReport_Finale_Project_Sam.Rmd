...
# Selection of Test set From 01-01-2017 to 10-03-2017
...
```{r}
reservation_per_day2<-reservations%>%filter(reserve_datetime>= ymd("20170101"), reserve_datetime<= ymd("20170310"))%>% group_by(ID,day_reservation,month_reservation,air_genre_name,air_area_name,latitude,longitude)%>%summarise(Tot_visit_day = sum(reserve_visitors), mean_visitors_per_day = mean(reserve_visitors))

reservation_per_day2<-reservation_per_day2%>%mutate(tot_reservation_per_day= Tot_visit_day/mean_visitors_per_day )
