...
---  
![](eseiaat-positiu-p3005-interior-blanc.png)
...
reservations %>%
  ...
  labs(x = "Air genre name", y = "Number of genre restaurants", 
       title= "Restaurants genre name frequency")
...
To be able to perform a proper analysis we modified some variables inside each data set, the very useful modification is related to the date. We take into consideration only the day related to the observation that we had, so we avoided considering also the time, this procedure helped us in making the analysis less struggling. 

From the analysis related to each data set we have some interesting results:
...
We also carried out an analysis related to the mean of visitors per month, but the result is useless because we have available only three months so we cannot state anything. So, one of the points that we will modify in the future will be related to the dimension of the observation that we have. We want to increase it to cover at least 1 entire year to catch information relate to all the year and ultimately a dataset that consists of many years.
...
In our report we mainly tried to use three models to create a prediction model:

- Linear regression
- Regression tree
- Random forest

To find the proper model to adopt we used an iterative approach, in which each time we wanted to understand the variables more significantly. This lead us to creating three variables that contribute a lot in increasing the variance explained by our model. The variables are:
...
The result obtained is ok and we believe we performed reasonably well in creating a model given the time that we had, but our work will not stop here in the future we will explore different models to try to create a model that allows us to reach an R-squared of 0.7 and higher.

Another future potential will be related to increasing the dimension of the information available, we want to collect more information that permits us to understand in a better way the behavior and habits of the population of the different areas, and collect information that shows a picture of the restaurants during all the year.
