...
```{r}
#add in data for avg of day of the week per id
...
mod10=lm( visitors ~  ID + day_of_week + holiday_flg + air_genre_name + air_area_name + count_of_reservations + Tot_visit_day_from_Reservations + avg_of_day_id, data=trainset2)
summary(mod10)
...
test the new fits with rsq 

```{r}


#mod 10
predictTest2<-predict(mod10, new_data = testset2)
#Calculation of SSE 
sse<-sum((testset$visitors-predictTest1)^2)
#Calculation of the average of Tot visi per day in the Test set
ybar2<- mean(testset$visitors)
