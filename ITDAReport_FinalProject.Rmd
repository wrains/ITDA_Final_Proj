...
submission <- submission %>% mutate(calendar_date = ymd(str_sub(ID,-10,-1))) %>% mutate(ID = str_sub(ID,1,-13)) %>% left_join(restaurant_info ,by="ID") %>% left_join(date_info ,by="calendar_date")%>% left_join(reservation_per_day %>% mutate(calendar_date=date),by=c("ID", "calendar_date"))
