...
combinedDataset=combinedDataset%>%select(-X.x,-ID.1,-day_mon_rest,-Tot_visit_day_from_Reservations,-count_of_reservations)
...
firsttree <- rpart(
  formula = visitors ~ ID+visit_date+air_area_name+latitude+longitude+air_genre_name+avgDailyVisitsGenre+date+Tot_visit_day_from_Reservations+count_of_reservations,
  data    = Train,
  method  = "anova"
  )
firsttree
...
submission <- submission %>% left_join(restaurant_info %>% select(-X),by="ID") %>% left_join(date_info %>% mutate(date = calendar_date),by="date")%>% left_join(reservation_per_day,by=c("ID", "date"))

submission$count_of_reservations[is.na(submission$count_of_reservations)] <- 0
submission$Tot_visit_day_from_Reservations[is.na(submission$Tot_visit_day_from_Reservations)] <- 0

submission$visitors <- predict(mod5up, newdata = submission)
```
